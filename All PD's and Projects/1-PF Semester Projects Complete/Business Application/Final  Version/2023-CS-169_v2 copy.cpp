...
void save_users_data(string user_Names_data[],string passwords_data[],string roles_data[], int &userName_Index){
    ...
    for (int i =0; i<userName_Index; i++)
    {
        file << user_Names_data[i] <<","<< passwords_data[i] <<","<<roles_data[i]<<endl;
    }
    file.close();
}
...
void saveProducts(string products[],int productPrice[],int productBoughtByUser[],int totalBoughtProducts[],int productIndex){
    ...
    for(int i=0;i<productIndex;i++){
        dataFile << products[i] << ","<< productPrice[i]<< ","<< productBoughtByUser[i]<< ","<< totalBoughtProducts[i] <<endl;
    }
    dataFile.close();
}
...
{
    fstream file;
    file.open("review_complains.txt" , ios :: out);
    for(int i = 0; i< review_index; i++){
        file << reviews[i];
        file << ",";
        file << rating[i];
...
void load_reviews(string reviews[], int rating[],int &review_index)
{
    fstream file;
    string line = "";
    file.open("review_complains.txt",ios::in);
    while(getline(file,line))
    {
        reviews[review_index] = parseData(line,1);
        rating[review_index] = convertStoInt((parseData(line,2)));
        review_index++;
    }
}
