...
main()
{
    ...
    load_items(products,ProductPrice,items_bought_by_user,total_bought_items,productIndex);
    ...
    save_items(products,ProductPrice,items_bought_by_user,total_bought_items,productIndex);
...
void Update_Rates(string products[], int productPrice[], int &productIndex, int &taxRate)
{
...
void editStock(string products[], int ProductPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex)
{
    ...
    // Keys
        ...
        else if (key == 13)
        {
            if (current_opt == 0)
            {
                add_item(products, ProductPrice, items_bought_by_user, total_bought_items, productIndex);
            }
            else if (current_opt == 1)
            {
...
void add_item(string products[], int ProductPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex)
...
void remove_item(string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int &productIndex)
{
    ...
    else
    {
        ...
        for (int i = index - 1; i < productIndex - 1; i++)     // loop above from the selected value to end of array
        {
            products[i] = products[i + 1];                          // Increments the index of each item by one
            productPrice[i] = productPrice[i + 1];
            items_bought_by_user[i] = items_bought_by_user[i + 1];
            total_bought_items[i] = total_bought_items[i + 1];
        }
        products[productIndex - 1] = "";                    // Places null at name and 0 at it's price
...
{                       
    ...
    for (int i = 0; i < productIndex; i++)
    {
        cout <<"\t\t\t\t\t\t\t"<< "|" << setw(25) <<left<< products[i] << setw(15) << ProductPrice[i] << setw(20) << "|"<<"\n";
    }
    cout <<"\t\t\t\t\t\t\t"<< "__________________________________________\n\n";
...
// Bill calculator for buoght items
...
// Customer Support Function where user write review, complain, contact us and check reviews
...
//  Both user and manager can use app settings function to change password and theme
...
// Save Items Function
void save_items(string products[], int productPrice[], int items_bought_by_user[], int total_bought_items[], int productIndex) {
    fstream dataFile;
...
// Load items function
void load_items(string products[],int Productprices[],int items_bought_by_user[],int total_bought_items[],int &productIndex){
    string data;
    productIndex = 0;
    fstream file;
